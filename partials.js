...
(function () {
  ...
  const mk = (href, svg, active) =>
    `<div class="rail-icon${active ? ' active' : ''}"${href ? ` style="cursor:pointer;" onclick="location.href='${href}'"` : ''}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${svg}</svg></div>`;
